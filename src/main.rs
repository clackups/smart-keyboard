...
pub(crate) struct InputCtx {
    ...
    /// Currently held mouse-button bitmask (bit 0 = left, bit 1 = right).
    /// Shared across all input sources so that a button held by one source
    /// is included in movement reports generated by any source.
    ...
    for evt in events {
        match evt.action {
            ...
            UserInputAction::Activate => {
                ...
                if *ctx.mouse_mode.borrow() {
                    let btns = {
                        ...
                        if evt.pressed { *mb |= 0x01; } else { *mb &= !0x01; }
                        *mb
                    };
            ...
            | UserInputAction::ActivateAltGr => {
                ...
                // ActivateShift = right mouse button; others are ignored.
                if *ctx.mouse_mode.borrow() {
                    if evt.action == UserInputAction::ActivateShift {
                        let btns = {
                            let mut mb = ctx.mouse_buttons.borrow_mut();
                            if evt.pressed { *mb |= 0x02; } else { *mb &= !0x02; }
                            *mb
                        };
                        ctx.hook.on_mouse_report(btns, 0, 0);
                    }
